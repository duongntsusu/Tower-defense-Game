...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif self. is_container ==True and ContainerAnt. can_contain(self, place. ant):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if hasattr(self, 'damage') and not hasattr(self, 'damage_doubled'):
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place!=None and place. is_hive ==False:
            ...
            # if the place contains one or more bees return a random one
            # if the place doesn't have any bees, place. bees =
                return random_bee(place. bees)
                # this function returns a random bee from a list of bees passed in or None
            place = place. entrance # move to the next place, which is current place's entrance
            distance +=1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        h = self. health - amount
        temp_place = self. place
        super(). reduce_health(amount)
        if h<=0: # if this reduction removes the bee:
            amount +=self. damage
        bee_s = list(temp_place. bees)
        for bee in bee_s:
...
class HungryAnt (Ant):
    ...
    def __init__ (self, health = 1):
        super(). __init__(health)
        self. cooldown = 0 # counts the number of turns it has left to chew
    def action(self, gamestate):
        if self. cooldown ==0: #nếu nó không chew, nó eat a random bee trong self. place. bees
            if self. place. bees:
...
class ContainerAnt(Ant):
    ...
    def __init__(self, health):
        super().__init__(health)
        self.ant_contained = None # this stores the ant it contains
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a: if our <ContainerAnt> currently contains an ant, <ant_contained>'s action is performed
...
# BEGIN Problem 9
class TankAnt (ContainerAnt):# containerAnt that deals 1 damage to all <Bee>'s in ints place each turn
    # also allow the ant that it contains to perform its action each turn
    name = 'Tank'
    food_cost = 6
    damage = 1
    implemented = True
    ...
    def action(self, gamestate):
        ...
        bee_s = list(self. place. bees)
        for bee in bee_s:
            bee. reduce_health (self. damage)
            # Damaging a bee will cause it to be removed from its place
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_waterproof:
            insect.reduce_health(insect.health)  # Instantly kill non-waterproof insects
        else:
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        super().__init__()
        if QueenAnt.has_been_placed:
            self.impostor = True
        else:
            self.impostor = False
            QueenAnt.has_been_placed = True
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        if self.impostor:
            self.reduce_health(self.health)  # Impostor dies instantly
            return
        ...
        while place:
            ...
            if ant:
                if ant not in self.doubled_ants:
                    ant.damage *= 2
                    self.doubled_ants.add(ant)
                if ant.is_container:
                    if ant.ant_contained and ant.ant_contained not in self.doubled_ants:
                        ant.ant_contained.damage *= 2
                        self.doubled_ants.add(ant.ant_contained)
            place = place.exit
        # END Problem 12
